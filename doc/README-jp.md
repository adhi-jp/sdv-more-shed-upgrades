...
この Mod に同梱されている `.tbin` マップデータファイルを差し替えることで、各シェッドの内部サイズを任意の大きさに変更できます。

カスタム `.tbin` ファイルを生成するための専用 Web アプリを用意しています。

**[SDV Shed Map Generator]**

[<img src="./sdv-shed-mapdata-generator.png" width="600" alt="SDV Shed Map Generator">][SDV Shed Map Generator]

【デフォルトの内部サイズ一覧】

| シェッド種別         | デフォルト幅 | デフォルト高さ | CanCaskHere |
| -------------------- | ------------ | -------------- | ----------- |
| デラックスシェッド   | 25           | 20             | No          |
| ギャラクシーシェッド | 31           | 26             | Yes         |
| インフィニティシェッド | 39           | 32             | Yes         |

【手順】

1. [SDV Shed Map Generator] を開く

2. **Shed Type** でカスタマイズしたいシェッドを選択する（Deluxe Shed / Galaxy Shed / Infinity Shed）

3. **Width**（幅）と **Height**（高さ）に任意の値を入力する

4. シェッド内で熟成ダルを設置可能にしたい場合は **CanCaskHere** にチェックを入れる

5. **Generate & Download** をクリックして `.tbin` ファイルをダウンロードする

6. ダウンロードした `.tbin` ファイルを `[CP] More Shed Upgrades/assets/Maps/` に配置し、既存のファイルを置き換える
...
> [!WARNING]
> `assets/Maps/` フォルダ内のカスタム `.tbin` ファイルは、Mod のアップデート時に上書きされます。アップデート後は、カスタマイズしたマップファイルを再度生成・配置してください。
